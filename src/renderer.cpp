...
std::vector<Triangle4d> Renderer::Clip(const std::vector<Triangle4d>& triangles,
    ...
    std::vector<Plane> planes = {
        ...
        Plane(
            Vector3d(-e / sqrt(e * e + 1), 0, -1 / sqrt(e * e + 1))),  // right
        Plane(
            Vector3d(0, e / sqrt(e * e + aspect_ratio * aspect_ratio),
                     -aspect_ratio /
                         sqrt(e * e + aspect_ratio * aspect_ratio))),  // bottom
        Plane(Vector3d(
            0, -e / sqrt(e * e + aspect_ratio * aspect_ratio),
...
std::vector<Triangle4d> Renderer::ToCube(std::vector<Triangle4d>& triangles,
    ...
    Matrix4d trans;
    trans << 2 * n / (r - l), 0, (r + l) / (r - l), 0, 0, 2 * n / (t - b),
        (t + b) / (t - b), 0, 0, 0, -1, -2 * n, 0, 0, -1, 0;
